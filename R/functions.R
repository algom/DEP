...
test_deqms <- function(se, type = c("control", "all", "manual"),
  ...
  if(!is.null(control)) {
    ...
    assertthat::assert_that(is.character(control),
                            length(control) == 1)
    if(!control %in% unique(col_data$condition)) {
      stop("run test_deqms() with a valid control.\nValid controls are: '",
           paste0(unique(col_data$condition), collapse = "', '"), "'",
           call. = FALSE)
    }
  }
  ...
  if(type == "control") {
    # Throw error if no control argument is present
    if(is.null(control))
      stop("run test_deqms(type = 'control') with a 'control' argument")
  ...
  if(type == "manual") {
    # Throw error if no test argument is present
    if(is.null(test)) {
      stop("run test_deqms(type = 'manual') with a 'test' argument")
    }
    assertthat::assert_that(is.character(test))

    if(any(!unlist(strsplit(test, "_vs_")) %in% conditions)) {
      stop("run test_deqms() with valid contrasts in 'test'",
           ".\nValid contrasts should contain combinations of: '",
           paste0(conditions, collapse = "', '"),
           "', for example '", paste0(conditions[1], "_vs_", conditions[2]),
           "'.", call. = FALSE)
  ...
  eB_fit <- eBayes(contrast_fit)
  #####################################
  ## Add DEqMS function. Adding the peptide count variance
  ############
  eB_fit$count = pep_table[rownames(eB_fit$coefficients),"count"]
  #check the values in the vector eB_fit$count
  #if min(eB_fit$count) return NA or 0, you should troubleshoot the error first
  min(eB_fit$count)
  ## [1] 1
  eB_fit = spectraCounteBayes(eB_fit)
  ##
  ##
  # function to retrieve the results of
  # the differential expression test using 'fdrtool'
